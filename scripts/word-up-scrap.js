...
function isDisallowedLetter(letter) {
    var field = model.currentAttempt;
    field = model.allowedLetters.indexOf(letter);
    if (field == -1){
        return true;
    } else if (field >= 0) {
        return false;
...
function containsOnlyAllowedLetters(word) {
    ...
    if (disallowedLettersInWord(word) == 0){
        return word;
...
var alreadyUsed;
    var a;
    var wordArray = model.wordSubmissions
    //a = wordArray.indexOf(word);
    //a = $.inArray( word , wordArray );
